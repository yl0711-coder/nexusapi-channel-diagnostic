...
Tokens 覆盖不全时只代表已报告样本的小计，不能据此计算完整费用。异常表逐接口、档位定位失败、不匹配及缺少回显；`verified` 与“存在少量不匹配”可以同时成立。`completed` 仅表示计划请求都已尝试，不代表渠道全部通过。运行记录逐轮展示已执行/计划、成功、失败和结束原因；中断或失败轮次的样本保留在 SQLite 中，但不进入小时质量图表，避免部分样本被误读为完整小时结果。旧轮次缺少计划请求数字段时显示“计划数未记录”，不补造历史分母。
...
每条样本立即提交；管理员 Ctrl-C/网页停止标为“管理员中断”，退出码 130；容器 SIGTERM 标为“系统中断”，退出码 143。两种中断都保留已完成样本并刷新 HTML。进程被强制结束时，控制服务在进程退出后或下次启动时将遗留 `running` 标为“异常中断”、补记结束时间；不重放旧请求。未知内部错误只记录异常类别，单轮命令非零退出，daemon 在下一个整点尝试新一轮。控制服务分别显示“等待整点”和“检测中”；定时检测启动后不能并发执行单轮，需先停止再点“立即执行一轮”。控制容器重启后只恢复先前明确启动且尚未停止的定时检测，不自动重放中断的单轮。环境代理与 HTTP 重定向均禁用；每次请求从开始连接到完整读取响应体的总墙钟时间分别受 `reasoning_timeout`、`juice_timeout` 限制，持续少量返回数据不会延长该期限；响应体最多读取 2 MiB。
